<template>
  <div class="page-container">
    <div class="page-header">
      ...
      <el-button type="primary" @click="openDialog()" class="btn-primary-sm">+ 新增仓库</el-button>
    </div>
    ...
    <el-dialog v-model="triggerVisible" title="选择 Commit 审核" width="860px">
      <div v-if="!selectedCommit">
        <div style="margin-bottom:16px">
          <span style="font-size:14px;color:var(--text-muted);margin-right:12px">选择分支：</span>
      ...
      <div v-else>
        <el-alert type="info" :closable="false" style="margin-bottom:16px;background:rgba(106,95,193,0.15);border-color:var(--purple-sentry);color:var(--text-secondary)">确认审核以下 Commit：</el-alert>
        <el-descriptions :column="2" border size="small">
          <el-descriptions-item label="分支">{{ selectedBranch }}</el-descriptions-item>
          <el-descriptions-item label="SHA"><code style="color:#c2ef4e">{{ selectedCommit.sha?.slice(0, 8) }}</code></el-descriptions-item>
...
.commit-row { padding: 12px 8px; border-bottom: 1px solid var(--purple-border); }
.commit-row:hover { background: rgba(106, 95, 193, 0.08); }
.commit-sha { font-family: monospace; color: var(--lime); font-size: 12px; }
